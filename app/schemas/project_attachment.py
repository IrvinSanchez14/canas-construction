...
Pydantic schemas for project attachment API requests and responses.
"""
...
class ProjectAttachmentBase(BaseModel):
    """Base schema for project attachment."""
    description: Optional[str] = Field(None, max_length=1000, description="Optional description")


class ProjectAttachmentCreate(ProjectAttachmentBase):
    """Schema for creating a project attachment (used internally after upload)."""
...
class ProjectAttachmentListResponse(BaseModel):
    ...
    attachments: list[ProjectAttachmentResponse] = Field(..., description="List of attachments")
    total: int = Field(..., description="Total count")
    max_allowed: int = Field(default=20, description="Maximum attachments per project")
